...
void init_simulation(particle_t* parts, int num_parts, double size) {
    ...
    iter_node = node_list;
    im_node_list = iter_node+bin_count;
}
...
void simulate_one_step(particle_t* parts, int num_parts, double size) {
	...
	for (i = 0; i < num_parts; ++i) {
		x = parts[i].x/cutoff;
		y = parts[i].y/cutoff;
        im_node_list[i].particle = &parts[i];
		#pragma omp atomic capture
        {   
			im_node_list[i].NextNode = iter_node[x*bin_size+y].NextNode;
			iter_node[x*bin_size+y].NextNode = &im_node_list[i];
        }
		parts[i].ax = 0;
		parts[i].ay = 0;
	}
